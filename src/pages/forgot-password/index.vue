...
const rules = reactive({
  email: [{ required: true, message: t('forgot-password.pleaseEnterEmail') }],
  code: [{ required: true, message: t('forgot-password.pleaseEnterCode') }],
  password: [{ required: true, message: t('forgot-password.pleaseEnterPassword') }],
  confirmPassword: [
    { required: true, message: t('forgot-password.pleaseEnterConfirmPassword') },
    {
      required: true,
      validator: validatorPassword,
      message: t('forgot-password.passwordsDoNotMatch'),
    },
  ] as FieldRule[],
})
...
async function getCode() {
  ...
  if (res.code === 0) {
    showNotify({
      type: 'success',
      message: `${t('forgot-password.sendCodeSuccess')}: ${res.result}`,
    })
  }
